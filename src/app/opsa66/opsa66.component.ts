...
@Component({
  selector: 'app-opsa',
  templateUrl: './opsa66.component.html',
  styleUrls: ['./opsa66.component.scss'],
  providers: [
    { provide: DateAdapter, useClass: AppDateAdapter },
    { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS }
  ]
})
export class OPSA66Component implements OnInit, AfterViewInit {
  ...
  getTRload(trdata) {
    this.configService.postdata2('opsa66/gettrload.php', { PEA_TR: trdata.PEA_TR }).subscribe((data => {
      if (data['status'] == 1) {
        var data = data['data'];
  ...
  getinfo() {
    this.configService.postdata2('opsa66/rdInfo.php', { data: 'roicdate' }).subscribe((data => {
      if (data['status'] == 1) {
  ...
  getvcaredate() {
    this.configService.postdata2('opsa66/rdvcaredate.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  writeDate(trdata, date) {
    this.configService.postdata2('opsa66/wriDate.php', { TRNumber: trdata, rundate: date }).subscribe((data => {
      if (data['status'] == 1) {
        this.getTrData();
        //  this.getStatus();
        // this.getJobProgressPea2();
  ...
  onOther(value, trdata) {
    this.configService.postdata2('opsa66/wriNote.php', { TRNumber: trdata, note: value }).subscribe((data => {
      if (data['status'] == 1) {
        this.getTrData();
        //  this.getStatus();
        this.getJobProgressPea2();
  ...
  getpeaList() {
    this.configService.postdata2('opsa66/rdpeaall2.php', {}).subscribe((data => {
  ...
  getpeaList2() {
    this.configService.postdata2('opsa66/rdpeaall.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  getLoad100() {
    this.configService.postdata2('opsa66/rdLoad100.php', { nDay: this.nDate }).subscribe((data => {
      if (data['status'] == 1) {
        // console.log(data['data']);
        var label = ['30 kVA']
        var nTR = [0];
  ...
  getDataRegion() {
    ...
    this.configService.postdata2('opsa66/rdTRAll.php', {}).subscribe((data => {
      if (data['status'] == 1) {
        this.trAllReg = data;
    ...
    regions.forEach(region => {
      this.configService.postdata('opsa66/rdProblemAll4.php', { region: region }).subscribe((data => {
        if (data['status'] == 1) {
  ...
  getDataRegionByProblem2() {
    ...
    var regions = Object.keys(GlobalConstants.regionLetter);
    regions.forEach(region => {
      this.configService.postdata('opsa66/rdLoadRegion2.php', { region: region }).subscribe((data => {
        if (data['status'] == 1) {
  ...
  getDataRegionByProblem() {
    ...
    var regions = Object.keys(GlobalConstants.regionLetter);
    regions.forEach(region => {
      this.configService.postdata('opsa66/rdLoadRegion4.php', { region: region }).subscribe((data => {
        if (data['status'] == 1) {
  ...
  getVcareChart() {
    ...
    var pea = [];
    var nVcare = [];
    this.configService.postdata2('opsa66/rdVcare.php', { peaCode: this.selPeapeaCode }).subscribe((data => {
      if (data['status'] == 1) {
        // get data
        console.log(this.peaname,)
  ...
  getJobProgressPea2() {
    ...
    this.configService.postdata2('opsa66/rdLoad5.php', { peaCode: this.selPeapeaCode, option: this.option, group: this.groupP1Select }).subscribe((data => {
      if (data['status'] == 1) {
  ...
  public getVcareData = () => {
    ...
    this.configService.getTr('opsa66/vcare.php?peaCode0=' + this.peaCode)
      //this.configService.getTr('TR.php?condition='+this.condition+'&peaCode0='+'B00000')
      .subscribe(res => {
        // this.dataSource.paginator = this.paginator1;
        // this.dataSource.sort = this.sort1;
  ...
  getMatReq() {
    this.configService.getMatReq('opsa66/getmatreq.php?nDay=' + this.nDate)
      //this.configService.getTr('TR.php?condition='+this.condition+'&peaCode0='+'B00000)
      .subscribe(res => {
  ...
  getMat(choice) {
    this.choice = choice;
    this.configService.postdata2('opsa66/rdMatSAP.php', { bat: this.bat }).subscribe((data => {
      if (data['status'] == 1) {
        // console.log(data);
  ...
  applyWBS(event) {
    if (window.confirm('คุณต้องการแก้ไขข้อมูล WBS/หมายเลขใบสั่ง ใช่หรือไม่?')) {
      this.configService.postdata2('opsa66/wriWBS.php', { TRNumber: event[1].PEA_TR, WBS: event[0] }).subscribe((data => {
        if (data['status'] == 1) {
          this.getTrData();
          //  this.getStatus();
          this.getJobProgressPea2();
  ...
  selectStatus(event) {
    // console.log(event);
    this.configService.postdata2('opsa66/wristatus.php', { TRNumber: event.value[1].PEA_TR, status: event.value[0] }).subscribe((data => {
      if (data['status'] == 1) {
        // console.log(data['data']);
        this.getTrData();
        //console.log(this.peaname);
  ...
  onSubmit() {
    ...
    this.configService.postdata2('opsa66/wriMat.php', this.registerForm.value).subscribe((data => {
      if (data['status'] == 1) {
        //this.getTrData();
        this.getMatReq();
  ...
  reTr(wbsdata) {
    ...
    this.configService.postdata2('opsa66/reTR2.php', wbsdata).subscribe((data => {
      if (data['status'] == 1) {
        // this.getData();
        this.getTrData();

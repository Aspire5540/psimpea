...
@Component({
  selector: 'app-opsa',
  templateUrl: './opsa.component.html',
  styleUrls: ['./opsa.component.scss'],
  providers: [
...
export class OPSAComponent implements OnInit, AfterViewInit {
  ...
  getvcaredate() {
    this.configService.postdata2('opsa/rdvcaredate.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  writeDate(trdata, date) {
    this.configService.postdata2('opsa/wriDate.php', { TRNumber: trdata, rundate: date }).subscribe((data => {
      if (data['status'] == 1) {
        this.getTrData();
        //  this.getStatus();
        // this.getJobProgressPea2();
  ...
  onOther(value, trdata) {
    this.configService.postdata2('opsa/wriNote.php', { TRNumber: trdata, note: value }).subscribe((data => {
      if (data['status'] == 1) {
        this.getTrData();
        //  this.getStatus();
        this.getJobProgressPea2();
  ...
  getDataRegion() {
    ...
    this.configService.postdata2('opsa/rdTRAll.php', {}).subscribe((data => {
      if (data['status'] == 1) {
        this.trAllReg = data;
  ...
  getVcareChart() {
    ...
    var pea = [];
    var nVcare = [];
    this.configService.postdata2('opsa/rdVcare.php', { peaCode: this.selPeapeaCode }).subscribe((data => {
      if (data['status'] == 1) {
        // get data
        console.log(this.peaname,)
  ...
  public getVcareData = () => {
    ...
    this.configService.getTr('opsa/vcare.php?peaCode0=' + this.peaCode)
      //this.configService.getTr('TR.php?condition='+this.condition+'&peaCode0='+'B00000')
      .subscribe(res => {
        // this.dataSource.paginator = this.paginator1;
        // this.dataSource.sort = this.sort1;
  ...
  getMatReq() {
    this.configService.getMatReq('opsa/getmatreq.php?nDay=' + this.nDate)
      //this.configService.getTr('TR.php?condition='+this.condition+'&peaCode0='+'B00000)
      .subscribe(res => {
  ...
  getMat(choice) {
    this.choice = choice;
    this.configService.postdata2('opsa/rdMatSAP.php', { bat: this.bat }).subscribe((data => {
      if (data['status'] == 1) {
        // console.log(data);
  ...
  applyWBS(event) {
    if (window.confirm('คุณต้องการแก้ไขข้อมูล WBS/หมายเลขใบสั่ง ใช่หรือไม่?')) {
      this.configService.postdata2('opsa/wriWBS.php', { TRNumber: event[1].PEA_TR, WBS: event[0] }).subscribe((data => {
        if (data['status'] == 1) {
          this.getTrData();
          //  this.getStatus();
          this.getJobProgressPea2();
  ...
  onSubmit() {
    ...
    this.configService.postdata2('opsa/wriMat.php', this.registerForm.value).subscribe((data => {
      if (data['status'] == 1) {
        //this.getTrData();
        this.getMatReq();

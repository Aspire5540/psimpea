...
@Component({
  selector: 'app-upload',
  templateUrl: './uploadopsa.component.html',
  styleUrls: ['./uploadopsa.component.scss']
})
export class UploadopsaComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.configService.postdata2('uploadsql/w048OPSAtosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  uploadOPSA() {
    this.configService.postdata2('uploadsql/opsatosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  uploadPM() {
    this.configService.postdata2('uploadsql/PMOPSAtosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {

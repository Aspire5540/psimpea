...
@Component({
  selector: 'app-uploadopsa4',
  templateUrl: './uploadopsa4.component.html',
  styleUrls: ['./uploadopsa4.component.scss']
})
export class Uploadopsa4Component implements OnInit {
  ...
  checkAutho() {
    ...
    if (this.peaCode == this.autoPeaCod || this.peaCode=='B00000') {
      return true;
    } else {
      return false;
    }
  ...
  onSubmit() {
    ...
    this.configService.postdata2('uploadsql/w048OPSA4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  uploadOPSA() {
    this.configService.postdata2('uploadsql/opsa4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  uploadPM() {
    this.configService.postdata2('uploadsql/PMOPSA4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
  ...
  uploadmb52() {
    this.configService.postdata2('uploadsql/mb52OPSA4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
        this.pmForm.resetForm();
        alert("เก็บข้อมูลแล้วเสร็จ");
      } else {
  ...
  uploadcn52n() {
    this.configService.postdata2('uploadsql/cn52nOPSA4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
        this.pmForm.resetForm();
        alert("เก็บข้อมูลแล้วเสร็จ");
      } else {
  ...
  uploadzcn52n() {
    this.configService.postdata2('uploadsql/zcn52nOPSA4tosql.php', {}).subscribe((data => {
      if (data['status'] == 1) {
        this.pmForm.resetForm();
        alert("เก็บข้อมูลแล้วเสร็จ");
      } else {
